            ...
            krs.getBlock = function (blockID, callback, pageRequest) {
                krs.sendRequest("getBlock" + (pageRequest ? "+" : ""), {
                    ...
                    if (response.errorCode && response.errorCode == -1 || !krs.constants || krs.constants.EPOCH_BEGINNING == 0) {
                        setTimeout(function () {
                            krs.getBlock(blockID, callback, pageRequest);
                        }, 2500);
                    } else {
                        if (callback) {
...
//区块列表
            krs.blocksPageLoaded = function (blocks) {
                ...
                for (var i = 0; i < blocks.length; i++) {
                    ...
                    rows += "<tr>" +
                        "<td><a href='#' data-block='" + krs.escapeRespStr(block.height) + "' data-blockid='" + krs.escapeRespStr(block.block) + "' class='block show_block_modal_action'" + (block.numberOfTransactions > 0 ? " style='font-weight:bold'" : "") + ">" + String(block.height).escapeHTML() + "</a></td>" +
